...
try {
  ...
  // Generate HTML output — pass config so embedded scripts use config.json values
  const htmlContent = generateHTML(outputData, searchAssertion, replaceEnabled, config);
...
function generateHTML(data, assertionName, replaceEnabled = false, cfg = {}) {
...
<body>
    ...
    <script>
        ...
        async function checkServerHealth() {
            try {
                const healthCheck = await fetch(SERVER_BASE + '/api/health', {
                    method: 'GET',
        ...
        function showServerError(error) {
            ...
            if (error.message === 'SERVER_NOT_RUNNING' || 
                ...
                errorMessage += 'The Gateway Utility server is not running.\\n\\n';
                errorMessage += 'To start the server:\\n\\n';
                errorMessage += '  1. Open a terminal/command prompt\\n';
                errorMessage += '  2. Navigate to the GatewayUtility directory\\n';
                errorMessage += '  3. Run: npm run server\\n\\n';
                errorMessage += 'You should see:\\n';
                errorMessage += '  Gateway Utility API server running at ' + SERVER_BASE + '\\n\\n';
        ...
        async function handleReplaceAssertions(searchAssertion) {
            ...
            try {
                ...
                const response = await fetch(SERVER_BASE + '/api/replace-assertions', {
                    method: 'POST',
                    headers: {
        ...
        async function handleImportBundles() {
            ...
            try {
                ...
                // Values injected from config.json at report-generation time
                const gateway = ${JSON.stringify(cfg.targetGateway || cfg.sourceGateway || '')};
                const schema  = ${JSON.stringify(cfg.importSchema  || '')};
                
                // Call the import API endpoint
                const response = await fetch(SERVER_BASE + '/api/import-bundles', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
